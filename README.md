<!--## About Me

I'm an aspiring game developer on an exciting journey to turn my passion into a career! 🎮✨

### My Game Development Journey

I'm actively working toward becoming a game developer, taking meaningful steps every day:

- 🎯 **Learning the fundamentals** - Building my skills in game design, programming, and development tools
- 🚀 **Making progress** - Each project and challenge brings me closer to my goal
- 💡 **Exploring creativity** - Combining technical skills with creative storytelling
- 🔨 **Building projects** - Working on game prototypes and learning from each iteration
- 🌱 **Growing continuously** - Embracing the learning process and pushing my boundaries

### What Drives Me

Game development represents the perfect fusion of creativity, technology, and storytelling. I'm passionate about creating immersive experiences that players can enjoy and remember. Every line of code, every design decision, and every completed project is a step forward on this path.

---

*"The journey of a thousand miles begins with a single step." - And I'm taking those steps every day!*
-->
![Image is just to look cool](https://images.alphacoders.com/133/thumb-1920-1337463.png)

# Hi there 👋

## I'm [Aaditya Raj](https://www.linkedin.com/in/aaditya-raj-32b30b37b/), a game developer from India.

> 💻 I make games with my free will and always ready to test some exciting features in my games.

🧑‍💻 My work on [Unity learning](https://unity.com/), C#, and Game designing.

🏢 Right now I'm currently working on some of my projects are [Dragons-Quest](https://github.com/Aadir0/Dragons_Quest).

🏫 I’m a Game Developer and Designer.

🤙 You can reach me through [LinkedIn](https://www.linkedin.com/in/aaditya-raj-32b30b37b/) and [Instagram](https://www.instagram.com/cholebhature_hehe/).
  
## ✌️ Fun fact:
  - I eat a lot 🍕.
  - I work with flow so sometimes I even forget to eat 🧑‍💼.
  - Hobbies: Gaming, Reading, Eating.
